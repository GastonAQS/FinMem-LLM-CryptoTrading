...
def sim_func(
    market_data_info_path: str = typer.Option(
        os.path.join("data", "env_data.pkl"),
        "-mdp",
        "--market-data-path",
        help="The environment data pickle path",
    ),
    start_time: str = typer.Option(
        "2024-10-14", "-st", "--start-time", help="The start time"
    ),
    end_time: str = typer.Option(
        "2024-10-31", "-et", "--end-time", help="The end time"
    ),
    run_mode: str = typer.Option(
        "test", "-rm", "--run-model", help="Run mode: train or test"
    ),
    config_path: str = typer.Option(
        os.path.join("config", "tsla_fingpt_config.toml"),
        "-cp",
        "--config-path",
